...
impl Config {
    ...
    ///
    /// **Note:** This ignores environment variables for self.target.
    pub fn apply_env(&mut self, cx: &mut ResolveContext) -> Result<()> {
        ...
        if let Some(future_incompat_report) = &mut self.future_incompat_report {
            future_incompat_report.apply_env(cx)?;
        }
...
impl BuildConfig {
    ...
    fn apply_env(&mut self, cx: &mut ResolveContext) -> Result<()> {
        ...
        if let Some(jobs) = cx.env_val("CARGO_BUILD_JOBS")? {
        ...
        if let Some(incremental) = cx.env_val("CARGO_INCREMENTAL")? {
        ...
        } else if let Some(incremental) = cx.env_val("CARGO_BUILD_INCREMENTAL")? {
